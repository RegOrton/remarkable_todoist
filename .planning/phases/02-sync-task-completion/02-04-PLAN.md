...
must_haves:
  truths:
    ...
    - "Refresh button pulls latest task state from Todoist (SYNC-01)"
    - "Completed task remains completed after refresh (sync verified)"
  artifacts:
    ...
    - path: "src/main.cpp"
      provides: "SyncManager QML registration"
      contains: "qmlRegisterUncreatableType"
  key_links:
    ...
    - from: "qml/main.qml"
      ...
      via: "Refresh button calls appController.refresh() which fetches latest tasks"
      pattern: "appController.refresh"
---

<objective>
Integrate SyncManager with AppController and wire up QML for task completion and sync status display.

Purpose: This is the final integration plan that connects all the pieces. The user taps a checkbox, the UI updates immediately (optimistic), and the sync happens in the background. The sync status indicator shows whether we're online and how many operations are pending. The existing refresh button continues to work, pulling latest task state from Todoist to verify sync success.

Output: Complete working task completion flow with sync status UI. Refresh button validates that completed tasks persist on server (SYNC-01).
</objective>
...
Note: The existing refresh() method already calls fetchAllTasks() which will pull the latest state from Todoist, including the completed status. This satisfies SYNC-01.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Add sync status indicator to QML header and register SyncManager</name>
  <files>qml/main.qml, src/main.cpp</files>
  <action>
Update main.qml to show sync status in header:

In main.qml, add sync status between the title and the flexible spacer in the header RowLayout:
...
In src/main.cpp:
- Add include: `#include "network/sync_manager.h"`
- Register SyncManager for QML (before engine.load()):
  `qmlRegisterUncreatableType<SyncManager>("RemarkableTodoist", 1, 0, "SyncManager", "Access via appController.syncManager");`
- This allows QML to access SyncManager's Q_PROPERTY values (isOnline, pendingCount, isSyncing)

Note: SyncManager is already accessible via appController.syncManager property, but registering the type ensures QML can understand and bind to its properties.
  </action>
  <verify>
Build with `cd /home/reg/Remarkable_Todoist/build && cmake .. && make -j$(nproc)` - no compile errors
Grep for "syncManager" in main.qml to confirm status indicator added
Grep for "qmlRegisterUncreatableType.*SyncManager" in main.cpp to confirm registration
  </verify>
  <done>Sync status indicator visible in header showing online/offline and pending count; SyncManager registered for QML</done>
</task>
...
- Refresh button pulls latest tasks from server (SYNC-01)
  </what-built>
  <how-to-verify>
...
6. Verify SYNC-01: Click "Refresh" button - tasks reload from server
7. Verify: Task remains completed after clicking Refresh (proves sync to server worked)
8. (Optional) Test offline: Disconnect network, complete task, verify "Offline" shown and pending count increases
  </how-to-verify>
...
3. SyncManager registered for QML in main.cpp
4. Sync status indicator visible and updates correctly
5. Task completion shows immediate visual feedback
6. Completed tasks sync to Todoist (visible in Todoist app/web)
7. Refresh button pulls latest state, completed tasks persist (SYNC-01)
</verification>
